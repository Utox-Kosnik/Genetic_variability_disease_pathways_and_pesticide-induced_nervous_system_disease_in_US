...
#Identify expanded set of top/bottom states to use for cross-validation
alz<- top.bot.dat[Disease %in% "Alzheimer's Disease" & Label == 2]
ms<- top.bot.dat[Disease %in% "Multiple Sclerosis" & Label == 2]
pd<- top.bot.dat[Disease %in% "Parkinson Disease" & Label == 2]

alz<- rbind(alz, tail(top.bot.dat[Disease %in% "Alzheimer's Disease" & Label == 1], 5))
ms<- rbind(ms, tail(top.bot.dat[Disease %in% "Multiple Sclerosis" & Label == 1], 5))
pd<- rbind(pd, tail(top.bot.dat[Disease %in% "Parkinson Disease" & Label == 1], 5))
...
#Not proceeding to ranked list - need full set for Figure 4
...
setwd("")
save.image(file=paste0("Workspaces/Step12b-Post_10Top_5Bottom_State_CrossValidation_Trial.RData"))
#load("Workspaces/Step12b-Post_10Top_5Bottom_State_CrossValidation_Trial.RData")
...
setwd("")
fwrite(cv.dat.out, file="SITables_Figures/SITable3_CV_FixedBottomState_Stats.csv")
...
#Copy of workspace available currently as "Fixed_PriorityList_Workspace.RData" used in subsequent steps
setwd("")
