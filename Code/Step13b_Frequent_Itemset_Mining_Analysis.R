...
#Identify and remove common pesticides implicated in rules
rule.rem<- usgs.year.pest[, .(Years=length(unique(YEAR))), by=.(State, CASRN, Compound_Simp)]
rule.rem<- rule.rem[Years >= 25, .(States=length(unique(State))), by=.(CASRN, Compound_Simp)]
rule.rem<- rule.rem[States >=45]
rule.rem<- mba.rules[Indiv_Pest %in% rule.rem$Compound_Simp]

mba.rules<- mba.rules[!Pest_Set %in% unique(rule.rem$Pest_Set)]
#Now only have rules not implicating common pesticides
length(unique(mba.rules$Pest_Set)) #1658309
...
setwd("")
jpeg("SITables_Figures/FigureS22a.jpeg", width=5, height=2.5, units="in", res=300)
all.venn.plot
graphics.off()
...
setwd("")
jpeg("SITables_Figures/FigureS22b.jpeg", width=5, height=7.5, units="in", res=300)
...
setwd("")
jpeg("SITables_Figures/FigureS22_Bars.jpeg", width=2, height=2.5, units="in", res=300)
...
setwd("")
fwrite(pest.type.bar.plot, "SITables_Figures/SI_FigureS22.csv")
